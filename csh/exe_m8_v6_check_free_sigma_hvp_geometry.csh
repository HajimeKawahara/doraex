...
if ( $#argv > 1 ) then
    echo "[m8_v6_sigma_hvp] expected no argument or --validate-only"
    exit 2
endif
if ( $#argv == 1 ) then
    if ( "$argv[1]" == "--m8-v6-sigma-hvp-gpu-lock-held" ) then
        set gpu_lock_held = 1
    else if ( "$argv[1]" == "--validate-only" ) then
        set gpu_lock_held = 1
        set validate_only = 1
    else
        echo "[m8_v6_sigma_hvp] unsupported argument: $argv[1]"
        exit 2
    endif
endif
...
if ( $gpu_lock_held == 0 ) then
    flock --exclusive --nonblock \
        ...
        "$gpu_lock" \
        /bin/tcsh -f "$script_path" --m8-v6-sigma-hvp-gpu-lock-held
    set lock_status = $status
    if ( $lock_status == $lock_conflict_status ) then
        echo "[m8_v6_sigma_hvp] not started: another Luhman 16B run holds $gpu_lock"
        echo "[m8_v6_sigma_hvp] wait for it to finish, then retry."
        exit 2
    endif
    exit $lock_status
endif

set diagnostic_script = "examples/luhman16b_yama/check_m8_v6_free_sigma_hvp_geometry.py"
set predecessor_script = "examples/luhman16b_yama/check_m8_v5_free_sigma_geometry.py"
set predecessor_launcher = "csh/exe_m8_v5_check_free_sigma_geometry.csh"
set predecessor_dir = "results/m8/v5/free_sigma_geometry"
set predecessor_summary = "$predecessor_dir/m8_v5_free_sigma_geometry_summary.json"
set predecessor_arrays = "$predecessor_dir/m8_v5_free_sigma_geometry_arrays.npz"
set predecessor_log = "$predecessor_dir/run.log"
set predecessor_provenance = "$predecessor_dir/provenance.sha256"
set predecessor_failed = "$predecessor_dir/FAILED"
...
set output_dir = "results/m8/v6/free_sigma_hvp_geometry"
set summary = "$output_dir/m8_v6_free_sigma_hvp_geometry_summary.json"
set arrays = "$output_dir/m8_v6_free_sigma_hvp_geometry_arrays.npz"
...
set gpu_python_pattern = 'python[0-9.]*[[:space:]].*examples/luhman16b_yama/(m[78](_p[0-9]+)?_v[0-9]+_run|run_milestone5_on_the_fly_atmosphere|make_m7_v1_linearization_check|check_m8_.*(linearization|convergence|validity|geometry|hvp))[.]py'
...
set expected_hashes = ( \
    ...
    "0f6fc7107eeac4c167e7078f968dd99f777bcd18092e9beefd24056735cd47b8" \
    ...
    "656cd49634104b24bf72d7903a0fac97c8e48dc822ef501c2421cbaa4dcd6b8a" \
    "359a02f0e664cb681fc92f1c82253ce0daad2a59e32177b5fd8d845744db07ca" \
...
if ( -e "$output_dir" ) then
    echo "[m8_v6_sigma_hvp] refusing output collision: $output_dir"
    exit 1
endif
if ( $#required_files != $#expected_hashes ) then
    echo "[m8_v6_sigma_hvp] internal provenance list mismatch"
    exit 1
endif
foreach required ( $required_files )
    if ( ! -f "$required" ) then
        echo "[m8_v6_sigma_hvp] missing required input: $required"
        exit 1
    endif
end
if ( ! -d "$database_dir" ) then
    echo "[m8_v6_sigma_hvp] missing molecular database: $database_dir"
    exit 1
endif
if ( ! -d "$opacity_cache_dir" ) then
    echo "[m8_v6_sigma_hvp] missing opacity cache: $opacity_cache_dir"
    exit 1
endif
...
while ( $hash_index <= $#required_files )
    ...
    if ( "$actual_hash" != "$expected_hashes[$hash_index]" ) then
        echo "[m8_v6_sigma_hvp] SHA256 mismatch: $required_files[$hash_index]"
        echo "[m8_v6_sigma_hvp] expected $expected_hashes[$hash_index]"
        echo "[m8_v6_sigma_hvp] actual   $actual_hash"
        exit 1
    endif
    @ hash_index++
end

# Force the frozen site-packages stack instead of the ambient ExoJAX checkout.
...
if ( $package_status != 0 ) then
    echo "[m8_v6_sigma_hvp] frozen package validation failed"
    exit $package_status
endif
...
set diagnostic_args = ( \
    ...
    --v5-summary "$predecessor_summary" \
    --v5-arrays "$predecessor_arrays" \
    --v5-failed "$predecessor_failed" \
    --out-dir "$output_dir" \
    --seed 0 \
...
if ( $validation_status != 0 ) then
    echo "[m8_v6_sigma_hvp] Python validation failed with status $validation_status"
    exit $validation_status
endif
if ( $validate_only == 1 ) then
    echo "[m8_v6_sigma_hvp] launcher validation passed; no GPU work was started"
    echo "[m8_v6_sigma_hvp] planned output: $output_dir"
    echo "[m8_v6_sigma_hvp] planned log: $log"
    echo "[m8_v6_sigma_hvp] nohup log must be outside the planned output directory"
    exit 0
endif

set active_gpu_pids = (`pgrep -f "$gpu_python_pattern"`)
if ( $#active_gpu_pids > 0 ) then
    echo "[m8_v6_sigma_hvp] refusing concurrent GPU work; active PID(s): $active_gpu_pids"
...
if ( $status != 0 ) then
    echo "[m8_v6_sigma_hvp] failed to create output parent: $output_parent"
    exit 1
endif
mkdir "$output_dir"
if ( $status != 0 ) then
    echo "[m8_v6_sigma_hvp] failed to claim fresh output directory: $output_dir"
    exit 1
endif
touch "$output_dir/RUNNING"
if ( $status != 0 ) then
    echo "[m8_v6_sigma_hvp] failed to create RUNNING sentinel"
    exit 1
endif

mkdir -p /tmp/numba-cache
mkdir -p /tmp/matplotlib-codex
mkdir -p /tmp/doraex-jax-cache-m8-v6-sigma-hvp
setenv NUMBA_CACHE_DIR /tmp/numba-cache
setenv MPLCONFIGDIR /tmp/matplotlib-codex
setenv JAX_COMPILATION_CACHE_DIR /tmp/doraex-jax-cache-m8-v6-sigma-hvp
...
echo "[m8_v6_sigma_hvp] start: `date`" | tee "$log"
echo "[m8_v6_sigma_hvp] log: $log" | tee -a "$log"
echo "[m8_v6_sigma_hvp] repository: $repo_root" | tee -a "$log"
echo "[m8_v6_sigma_hvp] git HEAD: `git rev-parse HEAD`" | tee -a "$log"
echo "[m8_v6_sigma_hvp] host: `hostname`" | tee -a "$log"
echo "[m8_v6_sigma_hvp] XLA autotune: production default (XLA_FLAGS unset)" | tee -a "$log"
echo "[m8_v6_sigma_hvp] molecular database: `realpath "$database_dir"`" | tee -a "$log"
echo "[m8_v6_sigma_hvp] opacity cache root: `realpath "$opacity_cache_dir"`" | tee -a "$log"
echo "[m8_v6_sigma_hvp] method: production directional HVP; no full Hessian; no FD fallback" | tee -a "$log"
...
if ( $backend_status != 0 ) then
    ...
    echo "[m8_v6_sigma_hvp] GPU/package preflight failed; refusing fallback." | tee -a "$log"
    exit $backend_status
endif
...
if ( $provenance_status != 0 || ! -s "$provenance_tmp" ) then
    mv "$output_dir/RUNNING" "$output_dir/FAILED"
    touch "$output_dir/FAILED"
    echo "[m8_v6_sigma_hvp] failed to write provenance hashes" | tee -a "$log"
...
if ( $status != 0 ) then
    mv "$output_dir/RUNNING" "$output_dir/FAILED"
    touch "$output_dir/FAILED"
    echo "[m8_v6_sigma_hvp] failed to finalize provenance hashes" | tee -a "$log"
    exit 1
endif
cat "$provenance" >> "$log"

echo "[m8_v6_sigma_hvp] diagnostic start: `date`" | tee -a "$log"
python -u "$diagnostic_script" $diagnostic_args >>& "$log"
set run_status = $status
if ( $run_status != 0 ) then
    mv "$output_dir/RUNNING" "$output_dir/FAILED"
    touch "$output_dir/FAILED"
    echo "[m8_v6_sigma_hvp] diagnostic failed with status ${run_status}: `date`" | tee -a "$log"
    exit $run_status
endif
if ( ! -f "$summary" || ! -f "$arrays" ) then
    mv "$output_dir/RUNNING" "$output_dir/FAILED"
    touch "$output_dir/FAILED"
    echo "[m8_v6_sigma_hvp] diagnostic returned without complete artifacts" | tee -a "$log"
    exit 1
endif

python -c 'import json, sys, numpy as np; summary=json.load(open(sys.argv[1], encoding="utf-8")); arrays=np.load(sys.argv[2], allow_pickle=False); required={"hvp_tangent","marker_sigma","marker_center_unconstrained","repeat_value","repeat_gradient","hvp_column_raw","fd_u_steps","fd_column_raw","analytic_gradient_u","analytic_hessian_uu","profile_sigma","profile_total_potential"}; missing=required-set(arrays.files); expected_shapes={"hvp_tangent":(12,),"marker_sigma":(4,),"marker_center_unconstrained":(4,12),"repeat_value":(4,5),"repeat_gradient":(4,5,12),"hvp_column_raw":(4,5,12),"fd_u_steps":(3,),"fd_column_raw":(4,3,12),"analytic_gradient_u":(4,),"analytic_hessian_uu":(4,)}; shapes=not missing and all(arrays[name].shape==shape for name,shape in expected_shapes.items()) and arrays["profile_sigma"].ndim==1 and arrays["profile_sigma"].shape==arrays["profile_total_potential"].shape; numeric=not missing and all(np.issubdtype(arrays[name].dtype,np.number) for name in required); finite=shapes and numeric and all(np.all(np.isfinite(arrays[name])) for name in required); expected_tangent=np.zeros(12,dtype=np.float32); expected_tangent[11]=1.0; tangent=shapes and np.array_equal(arrays["hvp_tangent"],expected_tangent); marker=np.asarray([0.27526917,0.32316479086875916,0.42335291,0.53313493]); steps=np.asarray([0.02,0.04,0.08]); config=summary.get("configuration",{}); config_ok=shapes and np.array_equal(arrays["marker_sigma"],marker) and np.array_equal(arrays["fd_u_steps"],steps) and config.get("repeat_count")==5 and np.array_equal(np.asarray(config.get("sigma_markers")),marker) and np.array_equal(np.asarray(config.get("fd_u_steps")),steps) and config.get("profile_num_actual")==arrays["profile_sigma"].size; method=summary.get("method",{}); method_ok=method.get("hvp_available") is True and method.get("hvp_fallback_allowed") is False and method.get("hvp_fallback_used") is False and method.get("full_hessian_computed") is False and method.get("direction_count")==1 and method.get("hvp_tangent_index")==11 and method.get("hvp_direction")=="u_sigma_log_p only" and method.get("hvp_method")=="jax.jvp(jax.value_and_grad(full_potential), e_u_sigma)"; ok=summary.get("execution_completed") is True and summary.get("calculation_integrity_passed") is True and not missing and shapes and numeric and finite and tangent and config_ok and method_ok; arrays.close(); print("artifact_gate",ok,sorted(missing),shapes,numeric,finite,tangent,config_ok,method_ok,summary.get("diagnostic_findings",{})); sys.exit(0 if ok else 5)' "$summary" "$arrays" >>& "$log"
set artifact_status = $status
if ( $artifact_status != 0 ) then
    mv "$output_dir/RUNNING" "$output_dir/FAILED"
    touch "$output_dir/FAILED"
    echo "[m8_v6_sigma_hvp] artifact calculation-integrity validation failed" | tee -a "$log"
...
if ( $output_hash_status != 0 || ! -s "$output_hashes_tmp" ) then
    mv "$output_dir/RUNNING" "$output_dir/FAILED"
    touch "$output_dir/FAILED"
    echo "[m8_v6_sigma_hvp] failed to write output hashes" | tee -a "$log"
...
if ( $status != 0 ) then
    mv "$output_dir/RUNNING" "$output_dir/FAILED"
    touch "$output_dir/FAILED"
    echo "[m8_v6_sigma_hvp] failed to finalize output hashes" | tee -a "$log"
    exit 1
endif
...
if ( $status != 0 ) then
    echo "[m8_v6_sigma_hvp] failed to mark completion" | tee -a "$log"
    exit 1
endif
touch "$output_dir/COMPLETE"
if ( $status != 0 ) then
    echo "[m8_v6_sigma_hvp] failed to timestamp completion" | tee -a "$log"
    exit 1
endif
echo "[m8_v6_sigma_hvp] summary: $summary" | tee -a "$log"
echo "[m8_v6_sigma_hvp] arrays: $arrays" | tee -a "$log"
echo "[m8_v6_sigma_hvp] done: `date`" | tee -a "$log"
exit 0
