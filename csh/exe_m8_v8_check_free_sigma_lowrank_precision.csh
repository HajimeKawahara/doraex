...
set output_dir = "results/m8/v8/free_sigma_lowrank_precision"
set nohup_parent = "results/m8/v8/nohup"
...
if ( $#argv > 1 ) then
    echo "[m8_v8_lowrank_precision] expected no argument, --validate-only, or --nohup"
    exit 2
endif
if ( $#argv == 1 ) then
    if ( "$argv[1]" == "--m8-v8-lowrank-precision-gpu-lock-held" ) then
        set gpu_lock_held = 1
        ...
        echo "[m8_v8_lowrank_precision] unsupported argument: $argv[1]"
        exit 2
    endif
endif
...
if ( $submit_nohup == 1 ) then
    if ( -e "$output_dir" ) then
        echo "[m8_v8_lowrank_precision] refusing output collision: $output_dir"
        exit 1
    endif
    mkdir -p "$nohup_parent"
    if ( $status != 0 ) then
        echo "[m8_v8_lowrank_precision] failed to create nohup log parent: $nohup_parent"
        exit 1
    endif
    set outer_log = `mktemp "$nohup_parent/m8_v8_lowrank_precision_nohup.XXXXXX.log"`
    set mktemp_status = $status
    if ( $mktemp_status != 0 || "$outer_log" == "" ) then
        echo "[m8_v8_lowrank_precision] failed to claim a unique nohup log"
        exit 1
    endif
    set outer_log = `realpath "$outer_log"`
    setenv DORAEX_M8_V8_OUTER_LOG "$outer_log"
    /usr/bin/nohup "$script_path" < /dev/null >&! "$outer_log" &
    set submit_status = $status
    set submitted_pid = $!
    unsetenv DORAEX_M8_V8_OUTER_LOG
    if ( $submit_status != 0 ) then
        echo "[m8_v8_lowrank_precision] nohup submission failed with status $submit_status"
        exit $submit_status
    endif
    echo "[m8_v8_lowrank_precision] submitted PID: $submitted_pid"
    echo "[m8_v8_lowrank_precision] outer log: $outer_log"
    echo "[m8_v8_lowrank_precision] run log: $repo_root/$output_dir/run.log"
    exit 0
endif
...
if ( $gpu_lock_held == 0 ) then
    flock --exclusive --nonblock \
        ...
        "$gpu_lock" \
        "$script_path" --m8-v8-lowrank-precision-gpu-lock-held
    set lock_status = $status
    if ( $lock_status == $lock_conflict_status ) then
        echo "[m8_v8_lowrank_precision] not started: another Luhman 16B run holds $gpu_lock"
        exit 2
    endif
    exit $lock_status
endif

set diagnostic_script = "examples/luhman16b_yama/check_m8_v8_free_sigma_lowrank_precision.py"
set expected_diagnostic_hash = "56dd6d57fde7a38d500ca9804e4904b420bb4ef806f5d33b80c5ef8846562d39"
set diagnostic_test = "tests/unittests/test_m8_free_sigma_lowrank_precision.py"
set v7_script = "examples/luhman16b_yama/check_m8_v7_free_sigma_derivative_capture.py"
set v7_launcher = "csh/exe_m8_v7_check_free_sigma_derivative_capture.csh"
set v7_test = "tests/unittests/test_m8_free_sigma_derivative_capture.py"
set v7_dir = "results/m8/v7/free_sigma_derivative_capture"
set v7_log = "$v7_dir/run.log"
set v7_provenance = "$v7_dir/provenance.sha256"
set v7_failure_hashes = "$v7_dir/failure_outputs.sha256"
set v7_failed = "$v7_dir/FAILED"
set v7_summary = "$v7_dir/m8_v7_free_sigma_derivative_capture_summary.json"
set v7_arrays = "$v7_dir/m8_v7_free_sigma_derivative_capture_arrays.npz"
set v7_checkpoint = "$v7_dir/m8_v7_free_sigma_derivative_capture_checkpoint.json"
set v7_outer_log = "results/m8/v7/nohup/m8_v7_sigma_capture_nohup.aVk1xx.log"
...
set summary = "$output_dir/m8_v8_free_sigma_lowrank_precision_summary.json"
set arrays = "$output_dir/m8_v8_free_sigma_lowrank_precision_arrays.npz"
set checkpoint = "$output_dir/m8_v8_free_sigma_lowrank_precision_checkpoint.json"
...
set gpu_python_pattern = 'python[0-9.]*[[:space:]].*examples/luhman16b_yama/(m[78](_p[0-9]+)?_v[0-9]+_run|run_milestone5_on_the_fly_atmosphere|make_m7_v1_linearization_check|check_m8_.*(linearization|convergence|validity|geometry|hvp|derivative_capture|lowrank_precision))[.]py'
...
set expected_hashes = ( \
    ...
    "ee499c78ed537fd5fc7cb1c6b630fcf416998e7eb1411edea14515f3413b4dde" \
    "016b7c3b314949180907526c9fd648b168fce0c5327da0973556865d0b06c1e2" \
    "688de079b3a5224ccf8d07953d6bb53d83a18c0d3f8bc5be2bcfaf0e90425ab1" \
    "f30a956a71f73739fa80d48f6a7c71fee579d77d7f94353b4fd8e64e33119049" \
    "606de2a88522c753241404bb190150bf148f2e611b6a12e0695c07addd8ef9db" \
    "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855" \
    "6eafdfc283ed726f4a88b5c4fbfccf9fd39ee66b72a91d001b00fa72a9db95fb" \
    "b6a292d3b0bca7eadfc664de74626e9ea75193f0d469c90dd1d1bcb3a228bb17" \
    "be11f6b14fbbee306fa4aa9998a9fb50a4562b0ddb1e5b66cbc69b4e0756debe" \
    "76c0f97ca374a15e8095d231a45527e2db6345d724acbe7b8c5ded4332780377" \
...
if ( "$expected_diagnostic_hash" == "REPLACE_WITH_FROZEN_M8_V8_PYTHON_SHA256" ) then
    echo "[m8_v8_lowrank_precision] diagnostic SHA256 is not frozen yet"
    exit 1
endif
if ( $#required_files != $#expected_hashes ) then
    echo "[m8_v8_lowrank_precision] internal provenance list mismatch"
    exit 1
endif
foreach required ( $required_files )
    if ( ! -f "$required" ) then
        echo "[m8_v8_lowrank_precision] missing required input: $required"
        exit 1
    endif
end
if ( -e "$v7_dir/RUNNING" || -e "$v7_dir/COMPLETE" || -e "$v7_dir/outputs.sha256" ) then
    echo "[m8_v8_lowrank_precision] v7 predecessor status no longer matches the pinned FAILED capture"
    exit 1
endif
if ( ! -d "$database_dir" || ! -d "$opacity_cache_dir" ) then
    echo "[m8_v8_lowrank_precision] molecular database or opacity cache is missing"
    exit 1
endif
if ( $?XLA_FLAGS ) then
    echo "[m8_v8_lowrank_precision] refusing pre-existing XLA_FLAGS drift: $XLA_FLAGS"
    exit 1
endif
...
while ( $hash_index <= $#required_files )
    ...
    if ( "$actual_hash" != "$expected_hashes[$hash_index]" ) then
        echo "[m8_v8_lowrank_precision] SHA256 mismatch: $required_files[$hash_index]"
        echo "[m8_v8_lowrank_precision] expected $expected_hashes[$hash_index]"
        echo "[m8_v8_lowrank_precision] actual   $actual_hash"
        exit 1
    endif
    @ hash_index++
end
sha256sum -c "$v7_provenance"
if ( $status != 0 ) then
    echo "[m8_v8_lowrank_precision] v7 provenance manifest validation failed"
    exit 1
endif
sha256sum -c "$v7_failure_hashes"
if ( $status != 0 ) then
    echo "[m8_v8_lowrank_precision] v7 failure artifact validation failed"
    exit 1
endif

setenv PYTHONPATH "$repo_root/src"
setenv PYTHONDONTWRITEBYTECODE 1
python -c 'import inspect, os, sys, jax, jaxlib, exojax, numpyro, numpy, scipy; import jax._src.config as jax_config; import jax._src.lax.lax as jax_lax; import scipy.linalg as scipy_linalg; import numpyro.distributions.continuous as continuous; import numpyro.distributions.transforms as transforms; import numpyro.infer.hmc_util as hmc_util; import numpyro.infer.initialization as initialization; import numpyro.infer.util as infer_util; modules=[jax,jaxlib,exojax,numpyro,continuous,numpy,scipy,scipy_linalg,hmc_util,initialization,infer_util,transforms,jax_config,jax_lax]; actual=[os.path.realpath(inspect.getfile(module)) for module in modules]; expected=[os.path.realpath(value) for value in sys.argv[7:21]]; versions=[jax.__version__,jaxlib.__version__,exojax.__version__,numpyro.__version__,numpy.__version__,scipy.__version__]; wanted=list(sys.argv[1:7]); print(f"versions={versions} paths={actual}"); sys.exit(0 if versions == wanted and actual == expected else 4)' "$expected_jax_version" "$expected_jaxlib_version" "$expected_exojax_version" "$expected_numpyro_version" "$expected_numpy_version" "$expected_scipy_version" "$expected_jax_path" "$expected_jaxlib_path" "$expected_exojax_path" "$expected_numpyro_path" "$numpyro_continuous_source" "$expected_numpy_path" "$expected_scipy_path" "$scipy_linalg_source" "$numpyro_hmc_source" "$numpyro_initialization_source" "$numpyro_util_source" "$numpyro_transforms_source" "$jax_config_source" "$jax_lax_source"
set package_status = $status
if ( $package_status != 0 ) then
    echo "[m8_v8_lowrank_precision] frozen package validation failed"
    exit $package_status
endif
...
if ( $validation_status != 0 ) then
    echo "[m8_v8_lowrank_precision] Python validation failed with status $validation_status"
    exit $validation_status
endif
if ( $validate_only == 1 ) then
    echo "[m8_v8_lowrank_precision] launcher validation passed; no GPU work was started"
    echo "[m8_v8_lowrank_precision] one-line submission: $script_path --nohup"
    echo "[m8_v8_lowrank_precision] planned output: $output_dir"
    echo "[m8_v8_lowrank_precision] planned run log: $log"
    exit 0
endif

set active_gpu_pids = (`pgrep -f "$gpu_python_pattern"`)
if ( $#active_gpu_pids > 0 ) then
    echo "[m8_v8_lowrank_precision] refusing concurrent GPU work; active PID(s): $active_gpu_pids"
...
if ( $status != 0 ) then
    echo "[m8_v8_lowrank_precision] failed to create output parent: $output_parent"
    exit 1
endif
mkdir "$output_dir"
if ( $status != 0 ) then
    echo "[m8_v8_lowrank_precision] failed to claim fresh output directory: $output_dir"
    exit 1
endif
touch "$output_dir/RUNNING"
if ( $status != 0 ) then
    echo "[m8_v8_lowrank_precision] failed to create RUNNING sentinel"
    exit 1
endif

mkdir -p /tmp/numba-cache
mkdir -p /tmp/matplotlib-codex
mkdir -p /tmp/doraex-jax-cache-m8-v8-lowrank-precision
setenv NUMBA_CACHE_DIR /tmp/numba-cache
setenv MPLCONFIGDIR /tmp/matplotlib-codex
setenv JAX_COMPILATION_CACHE_DIR /tmp/doraex-jax-cache-m8-v8-lowrank-precision
setenv PYTHONUNBUFFERED 1
setenv XLA_PYTHON_CLIENT_PREALLOCATE false

echo "[m8_v8_lowrank_precision] start: `date`" | tee "$log"
echo "[m8_v8_lowrank_precision] log: $log" | tee -a "$log"
if ( $?DORAEX_M8_V8_OUTER_LOG ) then
    echo "[m8_v8_lowrank_precision] outer nohup log: $DORAEX_M8_V8_OUTER_LOG" | tee -a "$log"
endif
echo "[m8_v8_lowrank_precision] repository: $repo_root" | tee -a "$log"
echo "[m8_v8_lowrank_precision] git HEAD: `git rev-parse HEAD`" | tee -a "$log"
echo "[m8_v8_lowrank_precision] host: `hostname`" | tee -a "$log"
echo "[m8_v8_lowrank_precision] XLA_FLAGS: unset (required)" | tee -a "$log"
if ( $?CUDA_VISIBLE_DEVICES ) then
    echo "[m8_v8_lowrank_precision] CUDA_VISIBLE_DEVICES: $CUDA_VISIBLE_DEVICES" | tee -a "$log"
else
    echo "[m8_v8_lowrank_precision] CUDA_VISIBLE_DEVICES: unset" | tee -a "$log"
endif
if ( $?JAX_PLATFORMS ) then
    echo "[m8_v8_lowrank_precision] JAX_PLATFORMS: $JAX_PLATFORMS" | tee -a "$log"
else
    echo "[m8_v8_lowrank_precision] JAX_PLATFORMS: unset" | tee -a "$log"
endif
echo "[m8_v8_lowrank_precision] completion gate: execution + method contract + artifact integrity only" | tee -a "$log"
python --version |& tee -a "$log"
git status --short --untracked-files=normal >>& "$log"

python -c 'import inspect, os, sys, jax, jaxlib, exojax, numpyro, numpy, scipy; import jax._src.config as jax_config; import jax._src.lax.lax as jax_lax; import scipy.linalg as scipy_linalg; import numpyro.distributions.continuous as continuous; import numpyro.distributions.transforms as transforms; import numpyro.infer.hmc_util as hmc_util; import numpyro.infer.initialization as initialization; import numpyro.infer.util as infer_util; backend=jax.default_backend(); devices=jax.devices(); modules=[jax,jaxlib,exojax,numpyro,continuous,numpy,scipy,scipy_linalg,hmc_util,initialization,infer_util,transforms,jax_config,jax_lax]; actual=[os.path.realpath(inspect.getfile(module)) for module in modules]; expected=[os.path.realpath(value) for value in sys.argv[7:21]]; versions=[jax.__version__,jaxlib.__version__,exojax.__version__,numpyro.__version__,numpy.__version__,scipy.__version__]; wanted=list(sys.argv[1:7]); print(f"versions={versions} paths={actual} backend={backend} devices={devices}"); gpu_only=backend=="gpu" and len(devices)>0 and all(device.platform=="gpu" for device in devices); sys.exit(0 if gpu_only and versions == wanted and actual == expected else 3)' "$expected_jax_version" "$expected_jaxlib_version" "$expected_exojax_version" "$expected_numpyro_version" "$expected_numpy_version" "$expected_scipy_version" "$expected_jax_path" "$expected_jaxlib_path" "$expected_exojax_path" "$expected_numpyro_path" "$numpyro_continuous_source" "$expected_numpy_path" "$expected_scipy_path" "$scipy_linalg_source" "$numpyro_hmc_source" "$numpyro_initialization_source" "$numpyro_util_source" "$numpyro_transforms_source" "$jax_config_source" "$jax_lax_source" >>& "$log"
set backend_status = $status
tail -n 1 "$log"
if ( $backend_status != 0 ) then
    mv "$output_dir/RUNNING" "$output_dir/FAILED"
    touch "$output_dir/FAILED"
    echo "[m8_v8_lowrank_precision] GPU/package preflight failed; refusing fallback" | tee -a "$log"
    exit $backend_status
endif
...
if ( $provenance_status != 0 || ! -s "$provenance_tmp" ) then
    mv "$output_dir/RUNNING" "$output_dir/FAILED"
    touch "$output_dir/FAILED"
    echo "[m8_v8_lowrank_precision] failed to write provenance hashes" | tee -a "$log"
...
if ( $provenance_check_status != 0 ) then
    mv "$output_dir/RUNNING" "$output_dir/FAILED"
    touch "$output_dir/FAILED"
    echo "[m8_v8_lowrank_precision] provenance self-check failed" | tee -a "$log"
    exit $provenance_check_status
endif

echo "[m8_v8_lowrank_precision] diagnostic start: `date`" | tee -a "$log"
python -u "$diagnostic_script" $diagnostic_args >>& "$log"
set run_status = $status
if ( $run_status != 0 ) then
    if ( -f "$arrays" && -f "$checkpoint" ) then
        python -c 'import json, sys; from pathlib import Path; from examples.luhman16b_yama import check_m8_v8_free_sigma_lowrank_precision as capture; result=capture.validate_checkpoint_artifacts(Path(sys.argv[1]),Path(sys.argv[2])); print(json.dumps(result,sort_keys=True)); sys.exit(0 if result.get("checkpoint_validation_passed") is True else 8)' "$checkpoint" "$arrays" >>& "$log"
        set checkpoint_status = $status
        if ( $checkpoint_status == 0 ) then
            set failure_artifacts = ( "$arrays" "$checkpoint" )
            if ( -f "$summary" ) set failure_artifacts = ( "$summary" $failure_artifacts )
    ...
    mv "$output_dir/RUNNING" "$output_dir/FAILED"
    touch "$output_dir/FAILED"
    echo "[m8_v8_lowrank_precision] diagnostic failed with status ${run_status}: `date`" | tee -a "$log"
    exit $run_status
endif
...
if ( ! -f "$summary" || ! -f "$arrays" || ! -f "$checkpoint" ) then
    mv "$output_dir/RUNNING" "$output_dir/FAILED"
    touch "$output_dir/FAILED"
    echo "[m8_v8_lowrank_precision] diagnostic returned without complete artifacts" | tee -a "$log"
    exit 1
endif

python -c 'import json, sys; from pathlib import Path; from examples.luhman16b_yama import check_m8_v8_free_sigma_lowrank_precision as capture; summary_path=Path(sys.argv[1]); result=capture.validate_saved_artifacts(summary_path,Path(sys.argv[2]),Path(sys.argv[3])); summary=json.loads(summary_path.read_text(encoding="utf-8")); hard_ok=(summary.get("execution_completed") is True and summary.get("method_contract_passed") is True and summary.get("artifact_integrity_passed") is True and result.get("artifact_integrity_passed") is True); print(json.dumps(result,sort_keys=True)); sys.exit(0 if hard_ok else 9)' "$summary" "$arrays" "$checkpoint" >>& "$log"
...
if ( $artifact_status != 0 ) then
    ...
    mv "$output_dir/RUNNING" "$output_dir/FAILED"
    touch "$output_dir/FAILED"
    echo "[m8_v8_lowrank_precision] artifact or method-contract validation failed" | tee -a "$log"
...
if ( $output_hash_status != 0 || ! -s "$output_hashes_tmp" ) then
    mv "$output_dir/RUNNING" "$output_dir/FAILED"
    touch "$output_dir/FAILED"
    echo "[m8_v8_lowrank_precision] failed to write output hashes" | tee -a "$log"
...
if ( $status != 0 ) then
    mv "$output_dir/RUNNING" "$output_dir/FAILED"
    touch "$output_dir/FAILED"
    echo "[m8_v8_lowrank_precision] output hash self-check failed" | tee -a "$log"
    exit 1
endif
...
echo "[m8_v8_lowrank_precision] done: `date`" | tee -a "$log"
