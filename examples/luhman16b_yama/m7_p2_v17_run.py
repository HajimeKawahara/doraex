"""Run the M7 P2 v17 direct-sigma coordinate-isolation pilot."""
...
DEFAULT_OUT_DIR = ROOT / "results" / "m7" / "p2" / "v17"
...
# M7-v1/P2 prior-CDF PCA rotation.  The values are embedded so this run source
# does not import any other m?-run implementation.  The posterior NPZ used to
...
def main():
    """Sample sigma_log_p directly while preserving the v15 target."""

    _ensure_option("--chip-indices", "0,1,2,3")
    _ensure_option("--out-dir", DEFAULT_OUT_DIR)
    _ensure_option(
        "--run-label",
        "m7_p2_v17_free_a_fixed_sigma_d_direct_sigma_log_p_full_dense",
    )
    ...
    # Match v15's active physical target: the seven bounded atmosphere
    # parameters, sigma_log_p, and four chip normalizations A are sampled;
    # sigma_d and log_w stay fixed at their v6 posterior medians.
    ...
    # The bounded seven-dimensional block retains its prior-CDF coordinates.
    # sigma_log_p keeps HalfNormal(0.3) physically, but is a separate positive
    # sample site so NumPyro uses its natural unconstrained log coordinate.
