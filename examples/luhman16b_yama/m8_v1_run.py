"""Run the M8 v1 production pressure-map retrieval."""
...
DEFAULT_OUT_DIR = ROOT / "results" / "m8" / "v1"
DEFAULT_INIT_FROM = ROOT / "results" / "m7" / "p2" / "v6" / "samples.npz"

# Seven-dimensional orthogonal rotation for the bounded atmospheric
# parameters.  This is the polar-orthogonalized bounded 7x7 block of the
# M7-v1/P2 prior-CDF PCA rotation.  The values are embedded so this production
# source does not import any m?-run implementation.  The posterior NPZ used to
# derive the original eight-dimensional rotation had SHA256:
...
def main():
    """Run the production chain using the validated M7 P2 v17 target."""
    ...
    # Preserve the validated v17 physical target.  The seven bounded
    # atmosphere parameters, sigma_log_p, and four chip normalizations A are
    # sampled. sigma_d and log_w remain fixed at their v6 posterior medians.
    _ensure_option("--fix-sigma-d")
    _ensure_option("--fix-log-w")
    _ensure_option("--dense-mass")

    # Production schedule.  Keeping warmup at depth 9 bounds adaptation cost;
    # retained samples use depth 11, matching the diagnostic series.
    _ensure_option("--no-preflight-autodiff")
    _ensure_option("--num-chains", 1)
    _ensure_option("--seed", 0)
    ...
    _ensure_option("--adapt-mass-matrix")
    _ensure_option("--no-x64")
    _ensure_option("--print-summary")

    # Keep the exact physical priors used in v17.  sigma_log_p retains its
    # HalfNormal(0.3) prior but uses NumPyro's direct positive coordinate.
    _ensure_option("--gaussianized-atmosphere")
    _ensure_option("--direct-sigma-log-p")
