"""Run the M7 P2 v13 fixed-A, free-sigma_d isolation pilot."""
...
DEFAULT_OUT_DIR = ROOT / "results" / "m7" / "p2" / "v13"
...
def main():
    """Fix A and log_w while sampling sigma_d and the atmosphere."""

    _ensure_option("--chip-indices", "0,1,2,3")
    _ensure_option("--out-dir", DEFAULT_OUT_DIR)
    _ensure_option("--run-label", "m7_p2_v13_fixed_a_free_sigma_d")
    ...
    # Relative to v8, release only sigma_d. A and log_w remain fixed at the
    # same component-wise v6 posterior medians used by that control run.
    _ensure_option("--fix-a")
