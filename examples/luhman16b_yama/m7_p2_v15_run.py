"""Run the M7 P2 v15 atmosphere+A full-dense isolation pilot."""
...
DEFAULT_OUT_DIR = ROOT / "results" / "m7" / "p2" / "v15"
...
def main():
    """Sample A with the atmosphere in one full dense-mass block."""

    _ensure_option("--chip-indices", "0,1,2,3")
    _ensure_option("--out-dir", DEFAULT_OUT_DIR)
    _ensure_option("--run-label", "m7_p2_v15_free_a_fixed_sigma_d_full_dense")
    ...
    # Relative to v14, change only the metric structure: A joins the eight
    # atmosphere coordinates in one 12-dimensional dense block. sigma_d and
    # log_w remain fixed at the same component-wise v6 posterior medians.
    _ensure_option("--fix-sigma-d")
    _ensure_option("--fix-log-w")
    _ensure_option("--dense-mass")

    # Match v14 exactly so this pilot isolates the mass-block change.
