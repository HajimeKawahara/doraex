"""Run the M7 P2 v16 700-warmup atmosphere+A full-dense pilot."""
...
DEFAULT_OUT_DIR = ROOT / "results" / "m7" / "p2" / "v16"
...
def main():
    """Adapt the atmosphere+A full dense mass over a 700-step warmup."""

    _ensure_option("--chip-indices", "0,1,2,3")
    _ensure_option("--out-dir", DEFAULT_OUT_DIR)
    _ensure_option(
        "--run-label",
        "m7_p2_v16_free_a_fixed_sigma_d_full_dense_warmup700",
    )
    ...
    # Match v15: A joins the eight atmosphere coordinates in one 12D dense
    # block, while sigma_d and log_w stay fixed at the v6 posterior medians.
    _ensure_option("--fix-sigma-d")
    _ensure_option("--fix-log-w")
    _ensure_option("--dense-mass")

    # The only scientific/sampling change from v15 is 200 -> 700 warmup.
    # NumPyro then estimates the final dense metric over a 400-draw window and
    # retains 50 draws for final step-size adaptation.
    _ensure_option("--no-preflight-autodiff")
    _ensure_option("--num-warmup", 700)
