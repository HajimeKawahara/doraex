"""Run the M7 P2 v10 fixed-log_w geometry-isolation pilot."""
...
DEFAULT_OUT_DIR = ROOT / "results" / "m7" / "p2" / "v10"
...
def main():
    """Run the dynamic-sigma pilot conditional only on fixed log_w."""

    _ensure_option("--chip-indices", "0,1,2,3")
    _ensure_option("--out-dir", DEFAULT_OUT_DIR)
    _ensure_option(
        "--run-label",
        "m7_p2_v10_dynamic_sigma_fixed_eigen_fixed_log_w",
    )
    ...
    # Fix only the chip-wise phase weights at their recentered v6 medians.
    # A and sigma_d remain sampled with the eight atmosphere coordinates.
    _ensure_option("--fix-log-w")
    _ensure_option("--dense-atmosphere-a-sigma-d-mass")

    # A 250-step schedule gives a final 100-point mass window for the
    # 16-dimensional dense block while keeping this conditional pilot bounded.
    _ensure_option("--no-preflight-autodiff")
    _ensure_option("--num-warmup", 250)
